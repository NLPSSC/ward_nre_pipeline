...
class TestErrorHandling:
    ...
    def test_document_generator_import_error(self, client, sample_text):
        ...
        with patch(
            "src.genalog_api.api.main.DocumentGenerator",
            side_effect=ImportError("genalog not installed"),
        ):
    ...
    def test_temp_directory_creation_error(
        ...
        with (
            patch("tempfile.NamedTemporaryFile") as mock_temp_file,
            patch(
                "tempfile.TemporaryDirectory", side_effect=OSError("Permission denied")
            ),
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.unlink"),
        ):
...
class TestEdgeCases:
    ...
    def test_special_characters_text(
        self, client, mock_document_generator, sample_image_bytes
    ):
    ...
    def test_extreme_dimensions(
        ...
        with (
            ...
            request_data = {"text": sample_text, "width": 10000, "height": 10000}
            response = client.post("/convert-text", json=request_data)
            assert response.status_code == status.HTTP_200_OK

            # Test very small dimensions
            request_data = {"text": sample_text, "width": 1, "height": 1}
            response = client.post("/convert-text", json=request_data)
            assert response.status_code == status.HTTP_200_OK

    def test_very_large_file_upload(
        self, client, mock_document_generator, sample_image_bytes
    ):
