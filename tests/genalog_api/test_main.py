...
class TestConvertTextEndpoint:
    ...
    def test_convert_text_success(
        ...
        with (
            patch("builtins.open", mock_open(read_data=sample_image_bytes)),
    ...
    def test_convert_text_default_parameters(
        self,
        client,
        sample_text,
        sample_base64_image,
    ...
    def test_convert_text_no_documents_generated(
    ...
    def test_convert_text_exception_handling(
        self, client, sample_text, mock_document_generator
    ):
        """Test exception handling in text conversion."""
        mock_document_generator.create_img_from_text.side_effect = Exception(
            "Test error"
        )
        ...
        with (
            ...
            assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert (
                "Error converting text to image: Test error"
    ...
    def test_convert_text_custom_parameters(
        self,
        client,
        sample_text,
        sample_base64_image,
...
class TestConvertFileEndpoint:
    ...
    def test_convert_file_with_parameters(
    ...
    def test_convert_file_unicode_decode_error(self, client):
        ...
        files = {
            "file": ("test.bin", io.BytesIO(binary_content), "application/octet-stream")
        }
    ...
    def test_convert_file_processing_error(
        self, client, sample_text, mock_document_generator
    ):
        ...
        with (
            ...
            assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert (
                "Error converting file to image: Processing error"
                in response.json()["detail"]
            )
